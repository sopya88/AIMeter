...
export default function Billing() {
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1100px] mx-auto">
      <div className="mb-5">
        <h1 className="text-lg md:text-xl font-semibold" style={{ color: "var(--text)" }}>Billing</h1>
        <p className="text-xs md:text-sm mt-0.5" style={{ color: "var(--muted)" }}>Invoices, chargebacks & budget tracker</p>
      </div>
      ...
      <div className="rounded-lg border p-4 md:p-5 mb-5"
        ...
        <div className="flex items-start justify-between mb-3 gap-2">
          ...
          <div className="text-right flex-shrink-0">
            <div className="text-lg md:text-xl font-semibold" style={{ color: "var(--text)" }}>{formatINR(totalSpend)}</div>
            <div className="text-xs" style={{ color: "var(--muted)" }}>of {formatINR(totalBudget)}</div>
          </div>
        </div>
              ...
              <tbody>
                {invoices.map((inv, i) => (
                  <tr key={inv.id}
                    ...
                    <td className="px-4 py-3 font-mono text-xs whitespace-nowrap" style={{ color: "var(--text)" }}>{inv.id}</td>
                    <td className="px-4 py-3 whitespace-nowrap" style={{ color: "var(--muted)" }}>{inv.date}</td>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap" style={{ color: "var(--text)" }}>{formatINR(inv.amount)}</td>
                    <td className="px-4 py-3 whitespace-nowrap" style={{ color: "var(--muted)" }}>{formatINR(inv.igst)}</td>
                    <td className="px-4 py-3 font-semibold whitespace-nowrap" style={{ color: "var(--text)" }}>{formatINR(inv.total)}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <button className="flex items-center gap-1 text-xs px-2 py-1 rounded border whitespace-nowrap"
              ...
              <tbody>
                {chargebacks.map((c, i) => (
                  <tr key={c.cc}
                    ...
                    <td className="px-4 py-3 font-mono text-xs whitespace-nowrap" style={{ color: "var(--muted)" }}>{c.cc}</td>
                    <td className="px-4 py-3 font-medium whitespace-nowrap" style={{ color: "var(--text)" }}>{c.team}</td>
                    <td className="px-4 py-3 whitespace-nowrap" style={{ color: "var(--text)" }}>{formatINR(c.openai)}</td>
                    <td className="px-4 py-3 whitespace-nowrap" style={{ color: "var(--text)" }}>{formatINR(c.anthropic)}</td>
                    <td className="px-4 py-3 whitespace-nowrap" style={{ color: "var(--text)" }}>{formatINR(c.google)}</td>
                    <td className="px-4 py-3 whitespace-nowrap" style={{ color: "var(--text)" }}>{formatINR(c.aws)}</td>
                    <td className="px-4 py-3 font-semibold whitespace-nowrap" style={{ color: "var(--text)" }}>{formatINR(c.total)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
