  ...
  return (
    <div className="p-4 md:p-6 max-w-[900px] mx-auto">
      <div className="mb-5">
        ...
        <p className="text-xs md:text-sm mt-0.5" style={{ color: "var(--muted)" }}>Company profile, billing & API keys</p>
      </div>
          ...
          {settings.apiKeys.map((k) => (
            <div key={k.provider} className="flex items-center justify-between p-3 rounded-lg border gap-3"
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {k.status === "connected"
                  ? <span className="hidden sm:flex items-center gap-1 text-xs" style={{ color: "#1D9E75" }}>
                      <CheckCircle size={13} />Connected
                    </span>
                  : <span className="hidden sm:flex items-center gap-1 text-xs" style={{ color: "#D97706" }}>
